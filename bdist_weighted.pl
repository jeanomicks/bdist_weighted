...
sub go {
    ...
    &calcCorr($dists, $mx, $flags, $nspec, $nchar, $nused, $weights, \*OF);
...
sub calcCorr {
        my ($dists, $mx, $flags, $nspec, $nchar, $nused, $w, $of) = @_;
        my %corr = ();
        ...
        for my $sp1 (sort keys %{$mx}) {
                my @a = ();
                for my $ssp1 (sort keys %{$mx}) {
                        push(@a, $dists->{$sp1}{$ssp1});
                }

                print $of "$sp1";
                for my $sp2 (sort keys %{$mx}) {
                        my @b = ();
                        for my $ssp2 (sort keys %{$mx}) {
                                push(@b, $dists->{$sp2}{$ssp2});
                        }
                        my $corr = &correlation(\@a, \@b, $#b+1);
                        print $of "\t$corr";
                }
                print $of "\n";
        }
...
sub correlation {
        my ($a, $b, $n) = @_;
        my $nn = $n - 1;
        my ($xx, $yy, $xy, $sumx, $sumy) = (0, 0, 0, 0, 0);
        for my $i (0 .. $nn) {
                $sumx += $a->[$i];
                $sumy += $b->[$i];
        }
